...
for fellow in all_fellows:
    with st.expander(f"Enter EM Shifts for {fellow}"):
        ...
        for i in range(num_em_ranges):
            em_entry = st.date_input(f"EM Shift #{i+1} Date or Range", key=f"{fellow}_em_entry_{i}", value=(start_date,))
...
for fellow in all_fellows:
    with st.expander(f"Enter Off-Days for {fellow}"):
        ...
        for i in range(num_off):
            off_entry = st.date_input(f"Off-Day #{i+1} (date or range)", key=f"{fellow}_off_{i}", value=(start_date,))
